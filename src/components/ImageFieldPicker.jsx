...
export default function ImageFieldPicker({
  ...
  const previewSrc = editorPreviewSrc(value, localImages)
  ...
  return (
    <div className={`space-y-3 ${compact ? '' : 'bg-white p-4 border border-gray-200 rounded-xl shadow-sm'}`}>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <label className="block text-xs font-extrabold text-[#0B1B3D]">{label}</label>
          {value && (
            <p className="text-[11px] text-gray-500 mt-0.5 font-mono truncate max-w-[240px]">{displayName}</p>
          )}
        </div>
        {value && (
          <button
      ...
      <div className="flex gap-4 items-start">
        <div
          className={`shrink-0 rounded-xl border-2 overflow-hidden bg-gray-50 flex items-center justify-center ${
            compact ? 'w-20 h-20' : 'w-28 h-28'
          } ${previewSrc ? 'border-gray-200' : 'border-dashed border-gray-300'}`}
        >
          {previewSrc ? (
            <img src={previewSrc} alt="" className="w-full h-full object-cover" />
          ) : (
            <span className="text-[10px] text-gray-400 font-semibold text-center px-2">No preview</span>
          )}
        </div>
                ...
                {filteredImages.length === 0 ? (
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
                    {filteredImages.map((preset) => {
                      ...
                      const thumb = templateThumbSrc(preset)
                      return (
                        <button
                          ...
                          title={preset.label}
                          className={`group relative aspect-square rounded-lg overflow-hidden border-2 transition focus:outline-none focus:ring-2 focus:ring-[#C8102E]/40 ${
                            selected
                              ? 'border-[#C8102E] ring-2 ring-[#C8102E]/30'
                              : 'border-transparent hover:border-[#C8102E]/50'
                          }`}
                        >
                          <img
                            src={thumb}
                            ...
                            loading="lazy"
                          />
                          <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent px-1 py-1">
                            <span className="block text-[9px] font-bold text-white truncate">{preset.label}</span>
                          </span>
                        </button>
                      )
                    })}
